...
public class Matriz {
	...
	public static SparseMatrix getMatrizP(){
		SparseMatrix mP = null;
		
		Random gerador = new Random();
		
		mP = new SparseMatrix(49290, k);
	...
	public static SparseMatrix getMatrizQ(){
		SparseMatrix mQ = null;		
		
		Random gerador = new Random();		
		//139738
		
		mQ = new SparseMatrix(139738, k);
	...
	public static double[][] getMatrizPDouble(){
		Random gerador = new Random();
		double[][] matrizP = new double[49290][k];
		for(int i = 0; i < 49290; i++){
			for(int j = 0; j < k; j++){
				if(gerador.nextInt(2) == 0 ){
					matrizP[i][j] = gerador.nextInt(6);
				}
			}
		}
		return matrizP;
	...
	public static double[][] getMatrizQDouble(){
		Random gerador = new Random();
		double[][] MatrizQ = new double[k][139738];
		for(int i = 0; i < k; i++){
			for(int j = 0; j < 139738; j++){
				if(gerador.nextInt(2) == 0 ){
					MatrizQ[i][j] = gerador.nextInt(6);
				}
			}
		}
		return MatrizQ;
	...
	public static double[][] multiplicacaoMatrizes(double[][] a, double[][] b){
		return null;
	}
	
	public static double[][] multiplicacaoMatrizPorNumero(double[][] a, double numero, int linha, int coluna){
		double[][] resultado = new double[linha][coluna];
		for(int i = 0; i < linha; i++){
			for(int j = 0; j < coluna; j++){
				resultado[i][j] = a[i][j]*numero;
			}
		}
		return resultado;
	}
	
	public static double[][] soma(double[][] a, double[][] b, int linha, int coluna){
		double[][] resultado = new double[linha][coluna];
		for(int i = 0; i < linha; i++){
			for(int j = 0; j < coluna; j++){
	...
	public static double[][] menos(double[][] a, double[][] b, int linha, int coluna){
		double[][] resultado = new double[linha][coluna];
		for(int i = 0; i < linha; i++){
			for(int j = 0; j < coluna; j++){
				resultado[i][j] = a[i][j] - b[i][j];
...
//	public static ArrayList<SparseMatrix> fatoracaoDouble(SparseMatrix mR, double[][] p, double[][] mQ, int iteracoes, double txA, double txB){
//		
//		ArrayList<SparseMatrix> lista = new ArrayList<SparseMatrix>();
//		
//		for(int n = 0; n < iteracoes; n++){
//			for(int i = 0; i < mR.numRows(); i++){				
//				//Vector linhai = p.viewRow(i);				
//				for(int j =0; j < mR.numCols(); j++){
//					if(mR.getQuick(i, j) > 0){						
//						Vector colunaTransQ = tranQ.viewColumn(j);						
//						double produto = 0;
//						for(int h = 0; h < k; h++){
//							produto = produto + p[i][k]*mQ[k][j];
//						}
//						
//						double erroij = mR.getQuick(i, j) - produto;
//						
//						for(int h = 0; h < k; h++){
//							
//							double [][] temp1p1 = multiplicacaoMatrizPorNumero(mQ, 2*erroij, k, j);
//							Vector temp1p = colunaTransQ.times(2*erroij);
//							
//							double temp2p2 = 
//							Vector temp2p = linhai.times(txB);
//							
//							Vector temp3p = temp1p.minus(temp2p);
//							
//							p.assignRow(i, linhai.plus(temp3p.times(txA)));
//							
//							
//							Vector temp1q = linhai.times(2*erroij);
//							
//							Vector temp2q = colunaTransQ.times(txB);
//							
//							Vector temp3q = temp1q.minus(temp2q);
//							
//							tranQ.assignColumn(j, colunaTransQ.plus(temp3q.times(txA)));
//						}
//					}						
//				}
//			}
...
//	}
	...
	public static ArrayList<SparseMatrix> fatoracao(SparseMatrix mR, SparseMatrix mP, SparseMatrix mQ, int iteracoes, double txA, double txB){
		SparseMatrix tranQ = (SparseMatrix) mQ.transpose();
		SparseMatrix p = mP;
		ArrayList<SparseMatrix> lista = new ArrayList<SparseMatrix>();
	...
	public static void main(String[] args) {
		...
		//0.002
		ArrayList<SparseMatrix> lista = fatoracao(getMatrizR(), getMatrizP(), getMatrizQ(), 500, 0.002, 0.01);
		SparseMatrix p = lista.get(1);
		SparseMatrix tranQ = lista.get(2);
		try {
